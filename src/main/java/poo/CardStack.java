...
//		cards.add(new MagicCard("yami", "/cards/59197169.jpg"));
		cards.add(new MagicCard("fissure", "/cards/66788016.jpg", new SpecialEffect() {
			@Override
			public void applyMagic(int player, Card card) {
				Field field;
				...
				List<Card> cards = field.getMonsterCards();

				MonsterCard lower = (MonsterCard) cards.get(0);

				for (Card value : cards) {
					MonsterCard monsterCard = (MonsterCard) value;
					if (monsterCard.getAttack() < lower.getAttack()) {
						lower = monsterCard;
					}
				}

				field.removeCard(lower);
			}
